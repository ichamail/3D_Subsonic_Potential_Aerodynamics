...
class AeroMesh(Mesh):
    ...
    def give_near_root_shells_id(self):
        ...
        if self.shells_ids:
            body_shells_ids = self.shells_ids["body"]
        else:
            body_shells_ids = np.arange(len(self.shells))
        
        near_root_shells_ids = []
        for shell_id in body_shells_ids:
            for node_id in self.shells[shell_id]:
                if node_id in near_root_nodes_id:
                    near_root_shells_ids.append(shell_id)
                    break
...
class PanelAeroMesh(AeroMesh, PanelMesh):
    ...
    def give_near_root_panels(self):
        near_root_shells_ids = super().give_near_root_shells_id()
        near_root_panels = []
        for id in near_root_shells_ids:
    ...
    def plot_mesh_inertial_frame(self, elevation=30, azimuth=-60,
        ...
        if self.shells_ids:
            ...
            if plot_wake:
                wake_panels = [
                    self.panels[id] for id in self.shells_ids["wake"]
                ]
        else:
    ...
    def copy(self):
        ...
        shells_ids = deepcopy(self.shells_ids)
        ...
        mesh = PanelAeroMesh(nodes, shells, nodes_ids, shells_ids, TrailingEdge,
